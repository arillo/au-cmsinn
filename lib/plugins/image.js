    ...
    Meteor.methods({
        '/au-cmsinn/image/resize' : function(options){
            ...
            if(_.isObject(image)){
                ...
                switch(gmm){
                    case 'crop': 
                        gm(buf).crop(dimensions[0], dimensions[1]).gravity('North').quality(100).toBuffer(function (err, buffer) {
...
Image = function(){
    this.hooks = {
        beforePublish: function(query, options, userId){
            ...
            options.fields['thumb.imageData'] = 0;
            options.fields['action.imageData'] = 0;
        }
    }
    this.storage = null;
    ...
    if(UI){
        ...
        UI.registerHelper('existImg', function (prefix, imageId, size) {
            ...
            if(image != undefined){
                ...
                if(Roles.userIsInRole(currentUser, ['admin','editor'])) return true;
                return false;
            }
