...
LabelPlugin = function(name, contentType, selector, ui, storage){
    ...
    this.language = 'en_US';
    this.defaultEmptyString = 'Empty_';
};
...
LabelPlugin.prototype.getRecordField = function(recordId, field){
    ...
    if(record){
        ...
        console.log(field);
        return this.defaultEmptyString+recordId;
    }
};
...
if(Meteor.isClient){
    ...
    if(UI) {
        UI.registerHelper('c', function (label, prefix) {
            ...
            var rs = '';
            if(parsedLabel[Utilities.CONST_RECORD_ID] === null){
                rs = CmsInnLabel.getRecord(parsedLabel[Utilities.CONST_ID]);
            } else {
                rs = CmsInnLabel.getRecordField(parsedLabel[Utilities.CONST_RECORD_ID], parsedLabel[Utilities.CONST_FIELD_ID]);
            }
            return rs;
        });
    } 
