...
if(Meteor.isClient){
    Utilities.isEditor = function(userId){
        return (Meteor.Device.isDesktop() || Meteor.Device.isTablet()) && Roles.userIsInRole(userId, ['admin','editor']);
    };
    Utilities.isAdmin = function(userId){
        return (Meteor.Device.isDesktop() || Meteor.Device.isTablet()) && Roles.userIsInRole(userId, ['admin']);
    };
}
