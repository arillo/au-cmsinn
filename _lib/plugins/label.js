...
LabelPlugin.prototype.disable = function($){
    ...
    // save changed labels
    _.each(this.labelsToSave.normal, _.bind(function(labelToSave){
        $(labelToSave.el).html('');
        this.upsertRecordField.apply(this, labelToSave.args);
    }, this));

    _.each(this.labelsToSave.selfContained, _.bind(function(labelToSave){
        $(labelToSave.el).html('');
        this.upsertRecord.apply(this, labelToSave.args);
    }, this));
};
    ...
    $(this.selector).on('hallodeactivated', function (e) {
        ...
        if(parsedLabel[Utilities.CONST_RECORD_ID] === null){
            self.labelsToSave.selfContained.push({
                el: e.currentTarget,
                args: [parsedLabel[Utilities.CONST_ID], newValue]
            });
        } else {
            self.labelsToSave.normal.push({
                el: e.currentTarget,
                args: [parsedLabel[Utilities.CONST_RECORD_ID], parsedLabel[Utilities.CONST_FIELD_ID], newValue]
            });
        }
    });
};
...
LabelPlugin.prototype.getRecordField = function(recordId, field){
    ...
    if(record){
        ...
        if(result && result[this.language]){
...
if(Meteor.isClient){
    ...
    if(UI) {
        ...
        UI.registerHelper('c', function (label, prefix) {
            contentDep.depend();
            var new_label = getRecordLabel(label, prefix);
            if(new_label == label.slice(1,-1) && !Utilities.isEditor(Meteor.userId())) return '';
            return new_label;
        });

        UI.registerHelper('c_empty', function (label, prefix) {
            contentDep.depend();
            if(Utilities.isEditor(Meteor.userId())) return false;
            var new_label = getRecordLabel(label, prefix);
            if(new_label == label.slice(1,-1)) return true;
            return false;
        });
